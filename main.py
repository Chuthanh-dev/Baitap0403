...
def main():
    ...
    """
    #Khởi tạo món ăn
    ga_ran = MonAn("Gà rán", 30000)
    hamburger = MonAn("Hamburger", 25000)
    cocacola = MonAn("Cocacola", 10000)
    #Khởi tạo hóa đơn
    hoa_don = HoaDon()

    print("Chào mừng các bạn đến với nhà hàng thức ăn nhanh !")
    print("Mời bạn nhập số lượng từng món ăn:")
    #Nhập số lượng món ăn từ người dùng
    so_luong_ga_ran = int(input(f"{ga_ran.ten}:"))
    so_luong_hamburger = int(input(f"{hamburger.ten}:"))
    so_luong_cocacola = int(input(f"{cocacola.ten}:"))
    #Thêm món ăn vào hóa đơn
    hoa_don.them_mon_an(ga_ran,so_luong_ga_ran)
    hoa_don.them_mon_an(hamburger,so_luong_hamburger)
    hoa_don.them_mon_an(cocacola,so_luong_cocacola)
    #in hóa đơn
    hoa_don.in_hoa_don()
